...
class PWCSE {
	...
	public function get_preview_html( $atts = '', $echo = false ) {
		...
		$output .= '<div class="pwc-configurator-parent-wrap '. esc_attr( implode( ' ', $el_classes ) ) .'">';
			...
			$output .= '<div id="configurator-' . esc_attr( $this->id ) . '" class="pwc-configurator">';
				...
				$output .= '<div id="configurator-view-' . esc_attr( $this->id ) . '" class="configurator-view pwc-configurator-view owl-carousel" '. implode( ' ', $slider_data ) .'>';
		...
		echo '<div class="config-cart-form-parent-wrap '. esc_attr( implode( ' ', $el_classes ) ) .'">';
			...
			if ( $product->is_in_stock() ) : ?>
				...
				<div class="config-cart-form config-cart-form-<?php echo esc_attr( $this->id ); ?>" data-config-id="<?php echo esc_attr( $this->id ); ?>">
					...
					<form class="cart <?php echo esc_attr( $overlap ); ?>" method="post" enctype='multipart/form-data'>
					...
					$output .= '<p>';
						...
						if( !empty( $active_components ) ) {
							...
							foreach ( $active_components as $key => $value ) {
								...
								$output .= '<span id="price-list-'. esc_attr( $uid ) .'" class="value" data-price="'. esc_attr( $price ) .'"> + '. esc_html( $parent_name ) .' '. esc_html( pwc_apply_currency_postion( $price ) ) .'</span>';
							}
	...
	public function product_share( $atts = '', $echo = false ) {
		...
		$output .= '<div class="product-share-wrap '. esc_attr( implode( ' ', $el_classes ) ) .'">';
			...
			$output .= '<div class="product-share" data-config-id="'. esc_attr( $this->id ) .'">';
				...
				if( in_array( 'google_plus', $social_share ) ) {
					$output .= '<a href="https://plus.google.com/share?url=" target="_blank" class="gplus '. apply_filters( 'pwc_icons', 'pwcf-google-plus' ) .'" title="'. esc_attr__( 'Google Plus', 'product-woo-configurator' ) .'"></a>';
				}
	...
	public function get_control_item( $value, $key, $data_attr_li = '' ) {
		...
		$html .= "<li " . $data_attr_li . " data-key='". esc_attr( $key ) ."' data-uid='". esc_attr( $value['uid'] ) ."' class='pwc-controls-list-img ". esc_attr( $class .' pwc-icon-'.$icon_type . $custom_icon_size_class ) ."'";
