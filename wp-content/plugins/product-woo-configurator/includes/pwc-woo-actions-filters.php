...
if( ! class_exists( 'PWC_Woo_Actions_Filters' ) ) {
	...
	class PWC_Woo_Actions_Filters {
		...
		public function add_configs_after_cart_item_name( $cart_item, $cart_item_key ) {
			...
			$output .= '<p class="pwc-inner pwc-child"><strong>'. esc_html__( 'Base Price', 'product-woo-configurator' ) . '</strong> - (' . pwc_apply_currency_postion( $base_price )  . ')</p>';
			...
			if ( $active_array && ! empty( $active_array ) ) {
				...
				foreach ( $active_array as $key => $value ) {				
					...
					if( isset( $value['parent'] ) && $value['parent'] ) {
						$output .= '<p class="pwc-inner pwc-child">'. sprintf( '<strong>%s</strong> - %s (%s)', $value['parent']['parent_name'], $value['name'], pwc_apply_currency_postion( $value['price'] ) ).'</p>';
					} else {							
						$output .= '<h4 class="pwc-title pwc-parent">'. sprintf( '%s (%s)', $value['name'], pwc_apply_currency_postion( $value['price'] ) ) .'</h4>';
					}				
		...
		public function add_order_items_meta( $item_id, $values, $cart_item_key ) {
			...
			if ( $active_array && ! empty( $active_array ) ) {
				...
				foreach ( $active_array as $value ) {
					...
					wc_add_order_item_meta( $item_id, $value['parent']['parent_name'], $value['name'] .' (' . pwc_apply_currency_postion( $value['price'] )  . ') ' );
				}
		...
		public function adjust_image_to_config( $product_image, $values, $cart_item_key ) {
			...
			if ( $active_array && ! empty( $active_array ) ) {
				...
				$output .= '<div class="pwc-config-image hide">';
		...
		public function add_to_cart( $url = false ) {
			...
			if( function_exists( 'WC' ) && $product_id ) {
				...
				if ( $passed_validation && WC()->cart->add_to_cart( $product_id, $quantity, 0, array(), array( 'user_config' => $active_array, 'config_id' => $config_id, 'pwc_base_price' => $base_price, 'views' => $views ) ) !== false ) {
