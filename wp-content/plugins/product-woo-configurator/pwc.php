...
if( is_plugin_active( 'woocommerce/woocommerce.php' ) || is_plugin_active_for_network( 'woocommerce/woocommerce.php' ) ) {
	...
	if( ! class_exists( 'PWC' ) ) {
		...
		class PWC {
			...
			public function licence_activation() {
				...
				if ( is_multisite() && ( is_plugin_active_for_network( 'product-woo-configurator/pwc.php' ) || is_network_only_plugin( 'product-woo-configurator/pwc.php' ) ) ) {
				...
				if( $status == false || $status != 'valid' ) {
					echo '<div class="notice notice-warning"><p>' . sprintf( __( 'Hola! Would you like to receive automatic updates and unlock premium support? Please <a href="%s">activate your copy</a> of Product Woo Configurator.', 'product-woo-configurator' ), wp_nonce_url( $redirect ) ) . '</p>' . '</div>';
				}
			...
			private function define_constants() {
				...
				define( 'PWC_VERSION', $plugin_data['Version'] );
	...
	function pwc_requirements_notice() {
		echo '<div class="updated"><p>' . __( 'Product WooCommerce Configurator requires <strong>WooCommerce</strong> to be installed and activated on your site.', 'product-woo-configurator' )  . '</p></div>';
	}
