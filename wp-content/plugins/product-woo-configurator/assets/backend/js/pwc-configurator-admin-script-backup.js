	...
	$.PWC.prototype = {
		...
		pwcRepeatSectionInsideSubValue : function ( self, repeatableField, structure ) {
			...
			if( $groupWrap.length == 0 ) {
				$groupWrap = $('<div />', { class:'pwc-settings-sub-value-group-wrapper' });
			...
			// console.log($repeatableFieldStructure.find('.pwc-sub-group-field').data('uid'));
		...
		getValFromLayers : function ( self ) {
			...
			var $layerOption = this.$layerOptions,
				...
				defaultVal = self.find('.field-default').val(),
				...
				configuratorPreview = self.find('.field-configurator-preview').val(),
				configuratorPreview = ( 'true' == configuratorPreview ) ? true : false,	
				groupViews = self.find('.field-group-views').val(),			
				controlStyle = self.find('.field-control-style').val(),			
				controlStyle = ( '' == controlStyle ) ? 'icon' : controlStyle,
			...
			$layerOption.find('.show-default').prop( 'checked', defaultVal);
			...
			$layerOption.find('.show-group-views').val(groupViews);
			$layerOption.find('.show-configurator-preview').prop( 'checked', configuratorPreview);
			$layerOption.find('.show-control-style').val(controlStyle);
		...
		onChangeVal : function ( self, val, fieldPrefix ) {
			...
			var con = self.data('con'),
				...
				$fieldCon = $('#pwc-settings').find('.active-layer').find('.'+con+'-con');

			fieldPrefix = ( typeof(fieldPrefix) == 'undefined' ) ? '' : fieldPrefix;

			// console.log( ( fieldPrefix ) ? fieldPrefix : 'empty' );
		...
		previewDragOptions : { 
			...
			stop: function( event, ui ) {
				...
				/*console.log( 'init-x:' + initX + ', init-y:' + initY );
		...
		createHotspot: function( $curPreview, uid ) {
			...
			var $transGroup = this.$tranformGroups,
				$hotspot = $('<div />', {class: 'pwc-hotspot', id: 'hotspot-'+ this.curImageView + '-' + uid, 'data-hotspot-uid': uid}).html('<span></span>');
		...
		bindEvents: function() {
			...
			$('#pwc-settings').on('click', '.trash-icon', function(e) {
				...
				if( $sure ) {
					var $parent = $(this).parent().parent().parent(), $uids = $parent.find('[data-uid]');
			...
			$('#pwc-settings-panel .select-image').on('preview:imageAdded', function(e, options) {
				...
				show = $(this).data('show');
				$('#pwc-settings').find('.active-layer').find('.'+show+'-con').data('src', options.imgurl );
			});

			$('#pwc-settings-panel').on('click', '.remove-img', function(e){
				
				var $imgCon = $(this).closest('.selected-con'), show;
			...
			pwc.$preview.on('preview:imageAdded', function(e, options) {
				...
				var uid, zIndex = '', divID, $imgCon, $curLayer, $typeCon, $transGroup = pwc.$tranformGroups;
				...
				if( ! $imgCon.length ) {
					$imgCon = $('<div />', {id: divID, class: 'pwc-preview-imgcon', 'data-preview-uid': uid });
			...
			$('#pwc-image-options').on('click', '#align-groups div div', function(e , $curImage, addedImgView, prefix ) {
				...
				if ( 'left' == value ) {
					...
					$activeImage.css({'left': '0px', 'right': 'auto' });
				
				} else if ( 'right' == value ) {

					$activeImage.css({'right': '0px', 'left': 'auto' });

				} else if ( 'center' == value ) {

					$activeImage.css({'left': ( ( pwc.previewWidth - $activeImage.width() ) / 2 ) + 'px', 'right': 'auto' });

					/*console.log( ( pwc.previewWidth - $activeImage.width() ) / 2 );

					console.log( pwc.previewWidth );

					console.log( $activeImage.width() );*/

				} else if ( 'middle' == value ) {

					$activeImage.css({'top': ( ( pwc.previewHeight - $activeImage.height() ) / 2 ) + 'px', 'bottom': 'auto' });
					...
					$activeImage.css({'top': '0px', 'bottom': 'auto' });
					pwc.$tranformGroups.find('#transform-y').val('0px').change();

				} else if ( 'bottom' == value ) {

					$activeImage.css({'bottom': '0px', 'top': 'auto' });
					pwc.$tranformGroups.find('#transform-y').val('0px').change();
				...
				$activeImage.attr( { 'data-init-x': $activeImage.position().left, 'data-init-y': $activeImage.position().top  } );

				// console.log( 'init-x:' + $activeImage.position().left + ', init-y:' + $activeImage.position().top );
			...
			// deselect layer
			/*$('#wpbody').on('click', function(e) {
				console.log( 'document click event triggered!' );
			});*/

			

			// deselect layers & images
			/*$(document).on("click", function(event){
				// deselect layer and images
				// deselect image
				// show overlay for all options except layers.
			});*/
